...
import { Role_FindOneDTO, Role_FindPermissionOfUserDTO, Role_InfoDTO, Role_UpdateDTO, Role_deleteDTO, RolesListResponse } from 'libs/erp/api-interfaces/src/modules/roles/RoleDTOs';
...
export class RoleService {
    ...
    getPermissionOfUserByEmail(email: string): Observable<Array<string>> {   
        const url = `${this.baseUrl}/email/`;
        const roleFindPermission: Role_FindPermissionOfUserDTO = {
            email: email,
        }
        return this.http.get<Array<string>>(url, { params: { ...roleFindPermission } }).pipe();
    }
