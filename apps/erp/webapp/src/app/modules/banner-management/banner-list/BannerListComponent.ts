...
import { Banner_FindDTO, Banner_InfoItemDTO } from "@global-victory/erp/api-interfaces";
...
@Component({
    ...
    templateUrl: './BannerListComponent.html',
    styleUrls: ['./BannerListComponent.css']
})
export class BannerListComponent implements AfterViewInit, OnInit {
    ...
    @ViewChild(BannerUpdateComponent) bannerUpdateComponent?: BannerUpdateComponent;
    @ViewChild(BannerDetailComponent) bannerDetailComponent?: BannerDetailComponent;
    ...
    loadData() {
        ...
        this.bannerService.getAllBanner(filter).subscribe((data) => {
            this.listBanner = data.result;
    ...
    showDetailBanner(id: string) {
        this.bannerDetailComponent?.detailBanner(id);
    }

    showFormUpdate(id: string) {
        this.bannerUpdateComponent?.showForm(id);
    }
